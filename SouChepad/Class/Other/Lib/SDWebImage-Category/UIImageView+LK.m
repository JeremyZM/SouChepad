...
- (void)setImageURL:(id)imageURL result:(void (^)(BOOL isDownload))isDownload
{
    [self sy_loadTapEvent];
    objc_setAssociatedObject(self, &imageURLKey, imageURL, OBJC_ASSOCIATION_COPY_NONATOMIC);
    if (imageURL)
    {
        [self reloadImageURLresult:^(BOOL download) {
            isDownload(download);
        }];
    }
    else
...
- (void)reloadImageURLresult:(void (^)(BOOL download))download;
{
    ...
    [self setImageWithURL:imageURL placeholderImage:nil options:(SDWebImageRetryFailed|SDWebImageLowPriority) progress:^(NSInteger receivedSize, NSInteger expectedSize) {
