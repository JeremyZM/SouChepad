...
{
    if ([self.dataSource respondsToSelector:@selector(multisectionIndexTitlesForTableView:level:)])
    {
        return [self.dataSource multisectionIndexTitlesForTableView:tableView level:[self indexOfTableView:tableView]];
    }
    return @[];
}
